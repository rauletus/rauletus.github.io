...
J'ai utilisé cette commande entre 1995 et 2000.
Elle était bon marché et facile à mettre en œuvre à une époque où les autres produits étaient plus complexes et plus coûteux. Elle fonctionne toujours et peut toujours être utilisée car elle était fondée sur les normes NMRA. Néanmoins, il faut bien admettre qu'elle est aujourd'hui dépassée, et c'est la raison pour laquelle j'utilise désormais une centrale basée sur le logiciel [DCC-EX](/techno/#dccex).
...
Donc rien ne va griller si une personne non avisée bascule le bouton G/HO-N de la commande MRC2000 sur la mauvaise position. En revanche, on constate un dégagement de température plus important au niveau des décodeurs qui se traduit par un échauffement de la caisse en plastique des locomotives qui avancent sensiblement plus vite. De même les phares (ampoules) éclairent davantage.  

Attention : Pour un signal électrique DCC (rectangulaire) sur la voie, un mutimètre **True RMS** avec une largeur de bande AC supérieure à 10 KHz (compte-tenu de la fréquence maxi du signal DCC) ou un oscilloscope est nécessaire pour avoir une mesure correcte du signal.